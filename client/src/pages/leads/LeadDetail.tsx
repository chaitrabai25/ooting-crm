  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="bg-white rounded-xl border border-slate-200/80 p-4 shadow-xs">
              ...
              <button
                ...
                className={`p-2.5 rounded-lg border text-center transition-all ${
                  isCurrent
                    ...
                    : isCompleted
                    ? 'bg-brand-50/60 text-brand-700 border-brand-200 font-semibold'
                    : 'bg-slate-50 text-slate-500 border-slate-200 hover:bg-slate-100 font-medium'
                }`}
              >
        ...
        <div className="mt-3 pt-3 border-t border-slate-100 flex justify-end gap-2 text-xs">
          <button
            type="button"
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-slate-700">{leadData.customer.phone}</span>
              </div>
              {leadData.customer.email && (
                <div className="flex items-center gap-2">
                  <Mail className="w-4 h-4 text-slate-400" />
                  <span className="text-slate-700">{leadData.customer.email}</span>
                </div>
              )}
              {leadData.customer.city && (
                <div className="flex items-center gap-2">
                  <MapPin className="w-4 h-4 text-slate-400" />
                  <span className="text-slate-700">{leadData.customer.city}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs">
            ...
            <div className="space-y-2.5 text-xs text-slate-700">
              <div className="flex justify-between py-1 border-b border-slate-100">
                ...
                <span className="font-semibold text-slate-900">{leadData.destination}</span>
              </div>
              <div className="flex justify-between py-1 border-b border-slate-100">
            ...
            {leadData.notes && (
              <div className="mt-4 pt-3 border-t border-slate-100 text-xs">
                ...
                <p className="text-slate-700 whitespace-pre-line bg-slate-50 p-2.5 rounded-lg border border-slate-200">
                  {leadData.notes}
                </p>
              </div>
            )}
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs">
            <div className="flex items-center justify-between mb-4">
              <div>
                <h3 className="text-sm font-bold text-slate-900">Follow-up Schedule</h3>
                <p className="text-xs text-slate-500">Planned phone calls, WhatsApp check-ins, and meetings</p>
              </div>
              <button
                type="button"
                onClick={() => setIsFollowUpOpen(true)}
          ...
          <div className="bg-white rounded-xl border border-slate-200/80 p-5 shadow-xs">
            <h3 className="text-sm font-bold text-slate-900 mb-4">Activity & Audit Timeline</h3>
            {timelineLogs.length === 0 ? (
              <p className="text-xs text-slate-500">No recorded timeline events yet.</p>
            ) : (
              <div className="relative pl-6 space-y-4 before:absolute before:left-2.5 before:top-2 before:bottom-2 before:w-0.5 before:bg-slate-200">
                {timelineLogs.map((log: any) => (
                  <div key={log.id} className="relative text-xs">
                    <div className="absolute -left-6 top-1 w-2.5 h-2.5 rounded-full bg-brand-600 ring-4 ring-brand-50" />
      ...
      >
        <form onSubmit={handleConvertBooking} className="space-y-4 text-xs">
          <div>
            <label className="font-semibold text-slate-700">Gross Package Amount (₹) *</label>
            <input
              type="number"
              required
              min="0"
              ...
              className="mt-1 w-full p-2.5 border border-slate-300 rounded-lg focus:ring-1 focus:ring-brand-500 focus:outline-none text-sm font-semibold"
            />
          </div>
          ...
          <div className="p-3 bg-brand-50 rounded-lg border border-brand-200 text-xs">
            <span className="font-bold text-slate-800">Final Amount Payable: </span>
      ...
      >
        <form onSubmit={handleScheduleFollowUp} className="space-y-4 text-xs">
          <div>
            <label className="font-semibold text-slate-700">Follow-up Date & Time *</label>
            <input
              type="datetime-local"
              required
              value={followUpDate}
          ...
          <div>
            <label className="font-semibold text-slate-700">Notes & Objective</label>
            <textarea
              rows={3}
              value={followUpNotes}
