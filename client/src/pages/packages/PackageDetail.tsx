...
export const PackageDetail: React.FC = () => {
  ...
  const handleImageFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onloadend = () => {
      if (typeof reader.result === 'string') {
        setDayImageUrl(reader.result);
      }
    };
    reader.readAsDataURL(file);
  };

  const handleSaveDayModal = (e: React.FormEvent) => {
    e.preventDefault();
    const newDay: ItineraryDay = {
      ...
      startTime: dayStartTime || null,
      endTime: dayEndTime || null,
    ...
    updated.sort((a, b) => a.dayNumber - b.dayNumber);
  ...
  const handlePersistItineraries = async () => {
    setIsSaving(true);
    try {
      await api.post('/packages/' + id + '/itineraries', { days: itineraries });
      alert('Itinerary changes saved successfully to database!');
      fetchPackage();
    } catch (err) {
        ...
        <form onSubmit={handleSaveDayModal} className="space-y-4 text-xs max-h-[78vh] overflow-y-auto pr-1">
          ...
          <div className="flex justify-end gap-2 pt-3 border-t border-slate-200">
            <button
              ...
              className="px-3.5 py-1.5 font-semibold text-slate-700 hover:bg-slate-100 rounded-xl transition-colors"
            >
              Cancel
            </button>
            <button
              type="submit"
              className="px-4 py-1.5 font-semibold text-white bg-[#C91F28] hover:bg-[#a81920] rounded-xl shadow-xs transition-colors"
            >
              {dayIndex !== null ? 'Update Day' : 'Add Day'}
            </button>
          </div>
        </form>
      </Modal>
